...
function estimateMarketValue(input: DealInput): {
  ...
  if (!valid.length) {
    // Previously this fell back to fairMarketValue: input.askingPrice, which
    // makes scoreValue's trueCost/fairMarketValue ratio self-referential
    // (~1 every time) and manufactures a mid-range Value score from zero
    // data -- the exact anchoring bias this product exists to defeat.
    // Returning 0 here is deliberate: it is not "the item is worthless", it
    // is "no valuation exists yet". scoreValue already treats
    // fairMarketValue <= 0 as unscoreable and returns 0, and analyzeDeal
    // uses valuationBasis to report "Insufficient Data" instead of a score
    // band, so this doesn't get read as "bad deal" either.
    assumptions.push("No usable comparable prices were supplied, so no fair market value could be estimated.");
    return {
      fairMarketValue: 0,
      comparableCount: 0,
      activeShare: 1,
...
function calculateConfidence(
  ...
  const raw = clamp(compScore + completeness + photos + conditionKnown + sellerEvidence, 0, 10);
  // With no valuation basis, nothing here should be read as "well
  // supported" no matter how complete the rest of the listing is -- cap at
  // 35%, same ceiling the standalone provider-layer rebuild verified.
  return valuationBasis === "unknown" ? Math.min(raw, 3.5) : raw;
}
...
function verdictFor(
  ...
  riskLevel: DealRecommendation["riskLevel"],
  valuationBasis: "comparables" | "unknown",
): DealRecommendation["verdict"] {
  // Protective overrides: a dangerous listing cannot be rescued by a cheap
  // price, and that holds even when there's no valuation either -- a known
  // danger outranks an unknown price.
  if (riskLevel === "Critical") return "Walk Away";
  if (riskLevel === "High" && score >= 60) return "High Risk";
  // No comparables means there is no honest verdict to give about the
  // price -- previously this fell through to the score bands below and
  // returned e.g. "Fair Deal" purely because scoreValue's self-referential
  // ratio (trueCost/askingPrice) manufactured a mid-range score from zero
  // data. Say so instead of guessing.
  if (valuationBasis === "unknown") return "Insufficient Data";
...
export function analyzeDeal(input: DealInput): DealRecommendation {
  ...
  // targetPrice first: openingOffer is defined relative to it below, not
  ...
  // Previously openingOffer was `askingPrice * (0.83 - leverage*0.08)`,
  // clamped up to askingPrice. On an overpriced listing the asking price is
  // not a meaningful anchor: e.g. asking $10,000 against a $5,000 fair
  // value opened around $8,300 -- ABOVE targetPrice ($4,525) -- so the
  // negotiation message offered high and then asked the seller to come
  // DOWN to a lower number than the offer itself.
  //
  // Anchor to whichever is lower: the asking price, or a modest premium
  // over fair market value. Then never let the opening offer exceed the
  // target -- an opener that starts above your own target isn't an opener.
  const anchor = Math.min(input.askingPrice, market.fairMarketValue * 1.05);
  const openingOffer = roundMoney(
    Math.min(
      targetPrice,
      clamp(anchor * (0.83 - leverage * 0.08), floor, anchor)
    )
  );
  ...
  // Both of these divide/subtract against market.fairMarketValue, which is
  // 0 when valuationBasis is "unknown" -- guard separately rather than
  // let a 0-vs-0 or Infinity ratio produce a misleading reason or a
  // negative "savings" figure.
  const estimatedSavings = market.valuationBasis === "unknown"
    ? 0
    : roundMoney(market.fairMarketValue - costs.trueCost);
  const confidencePercent = Math.round(confidenceScore * 10);

  const reasons: string[] = [];
  if (market.valuationBasis === "unknown") {
    reasons.push("No fair market value could be estimated from the data supplied, so price and savings figures are not meaningful yet.");
  } else {
  ...
  return {
    dealScore,
    verdict: verdictFor(dealScore, risk.riskLevel, market.valuationBasis),
